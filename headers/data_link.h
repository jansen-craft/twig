...
void print_ethernet(struct pcap_packet_header *ph, struct ethernet_header *eh){
	printf("%10u.%06u000\t%u\t%u\t", ph->ts_secs, ph->ts_usecs, ph->captured_length, ph->length);
	print_ethernet_address(eh->destination);
	printf("\t");
	print_ethernet_address(eh->source);
	printf("\t");
	printf("0x%04x\n", ntohs(eh->type));
}
